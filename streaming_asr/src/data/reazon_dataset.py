...
class ReazonDataset(IterableDataset):
    ...
    def __init__(self, text_encoder: TextEncoder, train: bool):
        if train:
            self.dataset = load_dataset("reazon-research/reazonspeech", streaming=True, name="all")["train"].skip(2400)  # type: ignore
        else:
            self.dataset = load_dataset("reazon-research/reazonspeech", streaming=True, name="all")["train"].take(2400)  # type: ignore
