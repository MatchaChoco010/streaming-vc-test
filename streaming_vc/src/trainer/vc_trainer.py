...
class Trainer:
    ...
    def __init__(
        ...
        self.optimizer_d_mel = optim.AdamW(self.d_mel.parameters(), lr=0.000005)
    ...
    def run(self):
        ...
        while self.step < self.max_step:
            ...
            xs = self.spk_rm(xs)
            xs = self.d_feat(xs)
            spk_rm_feat_loss = F.binary_cross_entropy(xs, torch.ones_like(xs)) * 40.0
            ...
            xs = self.mel_gen(xs)
            xs = self.d_mel(xs)
            spk_rm_mel_loss = F.binary_cross_entropy(xs, torch.ones_like(xs)) * 10.0
