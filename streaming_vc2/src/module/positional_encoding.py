...
class ChunkPositionalEncoding(nn.Module):
    """
    Chunk単位のPositional Encodingモジュール。
    """
    ...
    def forward(self, xs: torch.Tensor) -> torch.Tensor:
        ...
        """
        pe = torch.zeros(xs.size(1), self.d_model)
        position = torch.arange(0, xs.size(1)) // self.chunk_size
        position = position.to(dtype=torch.float32).unsqueeze(1)
        div_term = torch.exp(
            torch.arange(0, self.d_model, 2, dtype=torch.float32)
            * -(math.log(10000.0) / self.d_model)
        )
        ...
        pe[:, 1::2] = torch.cos(position * div_term)
        pe = pe.unsqueeze(0)
        pe = pe.to(device=xs.device, dtype=xs.dtype)
        return xs * self.xs_scale + pe[:, : xs.size(1)]
